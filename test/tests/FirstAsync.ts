...
describe("firstAsync", () => {
    itEnumerableAsync("FirstPredicate", async (asEnumerable) => {
        expect(await asEnumerable([1, 2]).firstAsync((x) => asPromise(x === 2)))
        .toBe(2)
    })
    ...
    itEnumerableAsync("empty array with predicate causes exception", async (asEnumerable) => {
        const promise = asEnumerable([1, 2, 3]).firstAsync((x) => asPromise(x === 4))
    ...
    itAsync("empty array with predicate causes exception async", async () => {
        const value = await expectAsync(asAsync([1, 2, 3]).firstAsync((x) => asPromise(x === 4)))
