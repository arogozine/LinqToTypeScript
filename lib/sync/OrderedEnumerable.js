...
const asKeyMap = (source, keySelector) => {
    const map = new Map();
...
function asSortedKeyValuesAsync(source, keySelector, ascending, comparer) {
    return __asyncGenerator(this, arguments, function* asSortedKeyValuesAsync_1() {
        const map = yield __await(asKeyMapAsync(source, keySelector));
...
const asKeyMapAsync = (source, keySelector) => __awaiter(this, void 0, void 0, function* () {
    const map = new Map();
    for (const item of source) {
        ...
        if (source instanceof OrderedEnumerable) {
            orderedPairs = function () {
                return __asyncGenerator(this, arguments, function* () {
                    for (const pair of source.orderedPairs()) {
                        yield __await(yield* __asyncDelegator(__asyncValues(asSortedKeyValuesAsync(pair, keySelector, ascending, comparer))));
                    }
                });
            };
        }
